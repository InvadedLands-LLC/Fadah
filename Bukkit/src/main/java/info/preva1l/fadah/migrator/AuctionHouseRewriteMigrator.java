...
import com.stacks.auctionhouse.AuctionHouse;
import com.stacks.modules.ListingType;
...
@Getter
public final class AuctionHouseRewriteMigrator implements Migrator {
    private final String migratorName = "AuctionHouseRewrite";

    @Override
    public List<Listing> migrateListings() {
        Map<Long, com.stacks.modules.Listing> listingMap = AuctionHouse.listings.getListings();
        ...
        for (long expiry : listingMap.keySet()) {
            com.stacks.modules.Listing listing = listingMap.get(expiry);
    ...
    public Map<UUID, List<CollectableItem>> migrateExpiredListings() {
        Map<Long, com.stacks.modules.Listing> listingMap = AuctionHouse.listings.getExpiredListings().getListings();
        Map<UUID, List<CollectableItem>> allItems = new ConcurrentHashMap<>();
        for (long added : listingMap.keySet()) {
            com.stacks.modules.Listing listing = listingMap.get(added);
            UUID owner = UUID.fromString(listing.getSeller_UUID());
            ItemStack itemStack = listing.getItem();
